...
function logger(func, logFunc) {
  return function log(...args) {
    const startArgs = JSON.stringify(args);
    logFunc(`${func.name}(${startArgs.slice(1, startArgs.length - 1)}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${startArgs.slice(1, startArgs.length - 1)}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function useArguments(...args2) {
